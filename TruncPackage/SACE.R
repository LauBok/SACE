SACE <- function(Z, S, Y, X, A, subset = NULL){
    ...
    X.set <- function(X){ 
        ...
        else { private.X <<- private.matrix.convert(X, "X"); private.checked <<- FALSE }
        return (private.X)
    }
    A.set <- function(A){ 
        if (is.null(A)) private.A <<- NULL
        else { private.A <<- private.matrix.convert(A, "A"); private.checked <<- FALSE }
        return (private.A)
    }
    ...
    #### Register Variables (Should only be used when private.checked = TRUE) ####
    ...
    return(list(GetZ = Z.get, GetS = S.get, GetY = Y.get, GetX = X.get, GetA = A.get, GetSubset = subset.get,
                ...
                PC = list(private.n.check,private.k.check,private.p.check,private.q.check), MIE = mie))
}

SIZE <- 60
Z.input <- rbinom(SIZE, 1, 0.5)
S1 <- rbinom(SIZE, 1, 0.8) # exposure
S2 <- rbinom(SIZE, 1, 0.4) # non-exposure
S.input <- Z.input * S1 + (1 - Z.input) * S2
Y1 <- rep(NA, SIZE)
Y1[S.input == 1] <- rnorm(SIZE)[S.input == 1]
Y1.input <- Y1
Y2.input <- matrix(rnorm(SIZE * 4,0,1), nrow = SIZE)
X.input <- matrix(rnorm(SIZE * 5,0,1), nrow = SIZE)
A.input <- rnorm(SIZE,0,1)

sace1 <- SACE(Z.input, S.input, Y1.input, X.input, A.input)
sace2 <- SACE(Z.input, S.input, Y2.input, X.input, A.input, rbinom(SIZE,1,0.75))
sace3 <- SACE(Z.input, S.input, Y2.input, X.input)
sace4 <- SACE(Z.input, S.input, Y1.input, A.input)
sace5 <- SACE(Z.input, S.input, Y2.input)
mie1 <- sace1$MIE()
mie2 <- sace2$MIE()
mie3 <- sace3$MIE()
mie4 <- sace4$MIE()
